...
for p_key in ["ddo", "sso", "mom"]:
    ...
    if f'up_key_{p_key}' not in st.session_state: st.session_state[f'up_key_{p_key}'] = 0
            ...
            if st.button("구글 시트에 반영", key=f"btn_{port_key}") and up_file:
                try:
                    ...
                    if not c_cols: st.error("🚨 파일에 '코드'라는 단어가 포함된 열이 없습니다.")
                        ...
                        st.session_state[f'editor_key_{port_key}'] += 1
                        st.session_state[f'up_key_{port_key}'] += 1
                        st.rerun()
...
with tabs[0]:
    ...
    for p_n, p_k in [("또", "ddo"), ("쏘", "sso"), ("맘", "mom")]:
        ...
        if not df.empty:
            ...
            df['p_tmp'] = df['종목코드'].apply(lambda x: global_prices.get(x, {}).get('prev', 0))
            buy, val, prev = (df['매수단가']*df['수량']).sum(), (df['c_tmp']*df['수량']).sum(), (df['p_tmp']*df['수량']).sum()
    ...
    if up_reb:
        try:
            ...
            if not c_cols or not m_cols:
                ...
                merged['코드'] = merged.apply(lambda r: f"https://finance.naver.com/item/main.naver?code={r['종목코드']}#{r['시장']}:{r['종목코드']}", axis=1)
                merged['차트'] = merged['종목코드'].apply(lambda x: f"https://m.stock.naver.com/fchart/domestic/stock/{x}")
