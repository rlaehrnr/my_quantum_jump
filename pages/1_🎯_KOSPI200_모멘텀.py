...
with tab1:
    ...
    # 💡 1. 연도 너비 1.2 복구 및 선정일 컬럼 삭제 (투자월 라벨과 통합)
    ...
    # 💡 2. 가장 마지막(최신) 달이 기본으로 선택되도록 인덱스 설정
    default_m_index = len(m_list) - 1 
    
    with c_m:
        # 선정일 글씨를 라디오 버튼 제목(투자 월) 옆에 넣기 위한 Placeholder
    ...
    if not df_monthly.empty:
        ...
        # 💡 3. 라디오 버튼 바로 위(제목 자리)에 "투자월" + "선정일" 을 합쳐서 표시
        month_label.markdown(f"**🌙 투자 월** <span style='font-size: 0.85rem; color: #9ca3af;'>&nbsp;&nbsp;💡 선정일: {base_date}</span>", unsafe_allow_html=True)
        ...
        with c_l:
            ...
            st.markdown(f"<h3 style='margin-bottom:0;'>🔥 퍼펙트 상승 <span style='font-size:15px; color:gray; font-weight:normal;'>(조건 일치: {count_p}개)</span></h3>", unsafe_allow_html=True)
            
            # 💡 4. 문장형 계산기 (상위 [ 입력창 ] 개 평균 : +XX%)
            cc1, cc2, cc3 = st.columns([1.0, 1.8, 6.0])
            with cc1: st.markdown("<div style='margin-top:6px; font-size:1.05rem; font-weight:bold; text-align:right;'>상위</div>", unsafe_allow_html=True)
            with cc2: top_n_p = st.number_input("p_n", 1, max(1, count_p), min(5, count_p) if count_p > 0 else 1, key="calc_p", label_visibility="collapsed")
            with cc3:
                if count_p > 0:
                    avg_ret_p = df_perf_t1.head(top_n_p)['이번달수익률'].mean()
                    avg_color_p = "#D32F2F" if avg_ret_p > 0 else "#1976D2"
                    st.markdown(f"<div style='margin-top:6px; font-size:1.05rem; font-weight:bold;'>개 평균 : <span style='color:{avg_color_p};'>{avg_ret_p:+.2f}%</span></div>", unsafe_allow_html=True)
                else:
                    st.markdown("<div style='margin-top:6px; font-size:1.05rem; font-weight:bold;'>개 평균 : <span style='color:gray;'>0.00%</span></div>", unsafe_allow_html=True)

            st.markdown('<p class="strategy-desc" style="margin-top: 5px;">1,3,6,12M 수익률 모두 상위 30% 이내 & 0보다 큰 종목 (3M 순)</p>', unsafe_allow_html=True)
            
            top5_p = df_perf_t1.head(top_n_p)['종목코드'].tolist() if count_p > 0 else []
            st.dataframe(df_perf_t1.style.apply(apply_k200_styling, highlight_codes=top5_p, axis=1), 
        ...
        with c_r:
            ...
            st.markdown(f"<h3 style='margin-bottom:0;'>🐎 달리는 말 <span style='font-size:15px; color:gray; font-weight:normal;'>(조건 일치: {count_s}개)</span></h3>", unsafe_allow_html=True)
            
            # 💡 4. 문장형 계산기 (상위 [ 입력창 ] 개 평균 : +XX%)
            cc4, cc5, cc6 = st.columns([1.0, 1.8, 6.0])
            with cc4: st.markdown("<div style='margin-top:6px; font-size:1.05rem; font-weight:bold; text-align:right;'>상위</div>", unsafe_allow_html=True)
            with cc5: top_n_s = st.number_input("s_n", 1, max(1, count_s), min(5, count_s) if count_s > 0 else 1, key="calc_s", label_visibility="collapsed")
            with cc6:
                if count_s > 0:
                    avg_ret_s = df_spec_t1.head(top_n_s)['이번달수익률'].mean()
                    avg_color_s = "#D32F2F" if avg_ret_s > 0 else "#1976D2"
                    st.markdown(f"<div style='margin-top:6px; font-size:1.05rem; font-weight:bold;'>개 평균 : <span style='color:{avg_color_s};'>{avg_ret_s:+.2f}%</span></div>", unsafe_allow_html=True)
                else:
                    st.markdown("<div style='margin-top:6px; font-size:1.05rem; font-weight:bold;'>개 평균 : <span style='color:gray;'>0.00%</span></div>", unsafe_allow_html=True)
                    
            st.markdown('<p class="strategy-desc" style="margin-top: 5px;">12M 수익률 상위 30% 이내 & 1M 수익률 상위 10% 이내 (1M 순)</p>', unsafe_allow_html=True)
            
            top5_s = df_spec_t1.head(top_n_s)['종목코드'].tolist() if count_s > 0 else []
            st.dataframe(df_spec_t1.style.apply(apply_k200_styling, highlight_codes=top5_s, axis=1), 
                         use_container_width=True, hide_index=True,
