...
with tab1:
    ...
    # 💡 3) 종목선정일 넓이를 1.5 -> 1.8로 늘려서 한 줄에 나오게 고정
    c_y, c_m, c_info = st.columns([1.2, 7.0, 1.8])
    ...
    if not df_monthly.empty:
        ...
        # 💡 3) <br> 태그를 지우고 margin-top을 35px로 맞춰서 라디오버튼과 완벽 수평 정렬
        with c_info: st.markdown(f"<div style='margin-top: 35px; text-align: right; color: #9ca3af;'>💡 <b>종목선정일:</b> {base_date}</div>", unsafe_allow_html=True)
        ...
        # 💡 1) 중간 정보 계기판 (코스피 수익률, 1·3M 하락, 대통령 연차, 투자진행) 완벽 복원
    ...
    if st.session_state.get('custom_run', False):
        with st.spinner("커스텀 가중치 시뮬레이션 연산 중..."):
            ...
            if not df_res_c.empty:
                ...
                st.dataframe(pd.DataFrame(stats_c), use_container_width=True, hide_index=True)
