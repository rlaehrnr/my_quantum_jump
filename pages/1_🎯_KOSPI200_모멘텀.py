...
with tab1:
    ...
    if not df_monthly.empty:
        ...
        with c_l:
            ...
            # 💡 제목 + 상위 N개 + 평균수익률 한 줄 배치
            col_t1, col_i1, col_r1 = st.columns([4.0, 2.0, 4.0])
            with col_t1: 
                st.markdown(f"<h4 style='margin-bottom:0; margin-top:2px;'>🔥 퍼펙트 상승 <span style='font-size:13px; color:gray; font-weight:normal;'>({count_p}개)</span></h4>", unsafe_allow_html=True)
            with col_i1: 
                top_n_p = st.number_input("p_n", 1, max(1, count_p), min(5, count_p) if count_p > 0 else 1, key="calc_p", label_visibility="collapsed")
            with col_r1:
                if count_p > 0:
                    ...
                    st.markdown("<div style='margin-top:8px; font-size:0.85rem; font-weight:bold; color:gray;'>상위 0개 평균: 0.00%</div>", unsafe_allow_html=True)

            st.markdown('<p class="strategy-desc" style="margin-top: 2px;">1,3,6,12M 수익률 모두 상위 30% 이내 & 0보다 큰 종목 (3M 순)</p>', unsafe_allow_html=True)
        ...
        with c_r:
            ...
            with col_t2: 
                st.markdown(f"<h4 style='margin-bottom:0; margin-top:2px;'>🐎 달리는 말 <span style='font-size:13px; color:gray; font-weight:normal;'>({count_s}개)</span></h4>", unsafe_allow_html=True)
            with col_i2: 
                top_n_s = st.number_input("s_n", 1, max(1, count_s), min(5, count_s) if count_s > 0 else 1, key="calc_s", label_visibility="collapsed")
            with col_r2:
                if count_s > 0:
            ...
            st.markdown('<p class="strategy-desc" style="margin-top: 2px;">12M 수익률 상위 30% 이내 & 1M 수익률 상위 10% 이내 (1M 순)</p>', unsafe_allow_html=True)
