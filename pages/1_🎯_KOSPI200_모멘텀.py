...
with tab1:
    ...
    if not df_monthly.empty:
        ...
        # 💡 수정됨: margin-bottom을 음수(-12px)로 주어 라디오 버튼을 투자연도 박스와 수평으로 딱 끌어올림!
        month_label.markdown(f"<div style='margin-bottom: -12px; margin-top: 2px;'>**🌙 투자 월** <span style='font-size: 0.85rem; color: #9ca3af;'>&nbsp;&nbsp;💡 선정일: {base_date}</span></div>", unsafe_allow_html=True)
        ...
        # 💡 [스마트 로직 1] 화면에 그리기 전, 사용자가 입력한 숫자(top_n_p, top_n_s)를 먼저 확보하고 UI 구성
        with c_l:
        ...
        # 💡 [스마트 로직 2] 선택된 숫자를 기준으로 교집합(겹치는 종목)만 추출!!
        ...
        # 💡 [스마트 로직 3] 위에서 구한 overlap_codes를 넘겨서 겹치는 종목만 빨갛게 칠하기
        with c_l:
