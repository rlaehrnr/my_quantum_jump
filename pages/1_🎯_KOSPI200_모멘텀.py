...
with tab1:
    ...
    if not df_monthly.empty:
        ...
        reason_desc = ("하락장" if is_bad_market else "") + (" + " if is_bad_market and is_below_ma else "") + ("4개월선 이탈" if is_below_ma else "")
        ...
        with c_l:
            st.dataframe(df_perf_t1.style.apply(apply_k200_styling, highlight_codes=df_perf_t1.head(top_n_p)['종목코드'].tolist(), overlap_codes=overlap_codes, axis=1), 
                         use_container_width=True, hide_index=True, column_order=['통합티커_L', '종목명_L', '1개월(%)', '3개월(%)', '6개월(%)', '12개월(%)', '이번달수익률'], column_config=main_cfg)
        with c_r:
            st.dataframe(df_spec_t1.style.apply(apply_k200_styling, highlight_codes=df_spec_t1.head(top_n_s)['종목코드'].tolist(), overlap_codes=overlap_codes, axis=1), 
...
with tab2:
    if os.path.exists(f_daily):
        ...
        status_d, box_d, text_d = ("🛑 투자 중지", "#FFEBEE", "#C62828") if (neg_1m_d >= 100 and neg_3m_d >= 100 or kospi_curr_d < kospi_mas_d.get(4, 0)) else ("✅ 투자 진행", "#E8F5E9", "#2E7D32")
        ...
        with col5d: st.markdown(f'<div style="background-color: #f0f2f6; padding: 10px; border-radius: 10px; text-align: center; border: 1px solid #d1d5db; height: 95px; display: flex; flex-direction: column; justify-content: center;"><div style="font-size: 13px; font-weight: bold; color: #333;">🇺🇸 실시간 모니터링</div><div style="font-size: 12px; color: #64748b;">로봇 수집일 기준</div></div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with c_chk:
        ...
        apply_timing = st.checkbox("🛑 마켓타이밍 적용 (개월선 이탈 OR 하락종목 100개↑ 시 현금)", value=True, key='t3_chk')
    ...
    with st.spinner("엔진 구동 중..."):
        ...
        if not df_res.empty:
            ...
            for col in s_cols:
                ...
                stats.append({"전략명": col, "CAGR (연평균)": f"{cagr:.1f}%", "총 누적수익률": f"{final_val-100:,.1f}%", "MDD (최대낙폭)": f"{mdd:.1f}%", "투자월 비율": f"{(df_res['invested'].sum()/len(df_res))*100:.1f}%", "월별 승률": f"{win_rate:.1f}%", "평균 수익률": f"{df_res.loc[df_res['invested'], col].mean():.2f}%"})
            st.dataframe(pd.DataFrame(stats).style.applymap(style_stats, subset=['CAGR (연평균)', '총 누적수익률', 'MDD (최대낙폭)']), use_container_width=True, hide_index=True)
...
with tab4:
    ...
    apply_timing_c = st.checkbox("🛑 마켓타이밍 적용", value=True, key='t4_chk_main')
    ...
    if st.session_state.get('custom_run', False):
        with st.spinner("커스텀 시뮬레이션 중..."):
            ...
            if not df_res_c.empty:
                ...
                with col_bc: st.download_button("📥 상세내역 다운로드", pd.DataFrame(trade_logs_c).to_csv(index=False).encode('utf-8-sig'), "K200_커스텀_백테스트.csv", "text/csv", use_container_width=True)
                ...
                stats_c = [{"전략명": "커스텀 스코어", "CAGR (연평균)": f"{cagr_c:.1f}%", "총 누적수익률": f"{final_val_c-100:,.1f}%", "MDD (최대낙폭)": f"{mdd_c:.1f}%", "투자월 비율": f"{(df_res_c['invested'].sum()/len(df_res_c))*100:.1f}%", "월별 승률": f"{(df_res_c.loc[df_res_c['invested'], '커스텀 전략']>0).mean()*100:.1f}%", "평균 수익률": f"{df_res_c.loc[df_res_c['invested'], '커스텀 전략'].mean():.2f}%"}]
                st.dataframe(pd.DataFrame(stats_c).style.applymap(style_stats, subset=['CAGR (연평균)', '총 누적수익률', 'MDD (최대낙폭)']), use_container_width=True, hide_index=True)
