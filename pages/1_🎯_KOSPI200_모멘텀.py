...
with tab1:
    ...
    target_month_str = f"{selected_year}-{m_list[0]}" # 초기값 방어
    ...
    if not df_monthly.empty:
        ...
        # 💡 [수정 1] 선정일을 연도 바로 옆 행에 배치 (높이 조절)
        with c_date: 
            st.markdown(f"<div style='margin-top: 38px; color: #475569; font-size: 0.9rem; font-weight: bold; background: #f1f5f9; padding: 5px 10px; border-radius: 6px; border: 1px solid #cbd5e1; white-space: nowrap;'>💡 선정일: {base_date}</div>", unsafe_allow_html=True)
        ...
        with c_l:
            ...
            st.markdown(f"### 🔥 퍼펙트 상승 <span style='font-size:15px; color:gray; font-weight:normal;'>(조건 일치: {count_p}개)</span>", unsafe_allow_html=True)
            
            # 다이나믹 입력기
            col_calc1, col_calc2 = st.columns([1.5, 2.5])
            with col_calc1:
                top_n_p = st.number_input("Top N 선택", 1, max(1, count_p), min(5, count_p) if count_p > 0 else 1, key="calc_p")
            
            if count_p > 0:
                avg_ret_p = df_perf_t1.head(top_n_p)['이번달수익률'].mean()
                avg_color_p = "#D32F2F" if avg_ret_p > 0 else "#1976D2"
                with col_calc2:
                    st.markdown(f"<div style='margin-top:28px; font-size:1.1rem; font-weight:bold;'>➔ 상위 {top_n_p}개 평균: <span style='color:{avg_color_p};'>{avg_ret_p:+.2f}%</span></div>", unsafe_allow_html=True)
            
            st.markdown('<p class="strategy-desc">1,3,6,12M 수익률 모두 상위 30% 이내 & 0보다 큰 종목 (3M 순)</p>', unsafe_allow_html=True)
            st.dataframe(df_perf_t1.style.apply(apply_k200_styling, axis=1), 
        ...
        with c_r:
            ...
            st.markdown(f"### 🐎 달리는 말 <span style='font-size:15px; color:gray; font-weight:normal;'>(조건 일치: {count_s}개)</span>", unsafe_allow_html=True)
            
            # 다이나믹 입력기
            col_calc3, col_calc4 = st.columns([1.5, 2.5])
            with col_calc3:
                top_n_s = st.number_input("Top N 선택", 1, max(1, count_s), min(5, count_s) if count_s > 0 else 1, key="calc_s")
            
            if count_s > 0:
                avg_ret_s = df_spec_t1.head(top_n_s)['이번달수익률'].mean()
                avg_color_s = "#D32F2F" if avg_ret_s > 0 else "#1976D2"
                with col_calc4:
                    st.markdown(f"<div style='margin-top:28px; font-size:1.1rem; font-weight:bold;'>➔ 상위 {top_n_s}개 평균: <span style='color:{avg_color_s};'>{avg_ret_s:+.2f}%</span></div>", unsafe_allow_html=True)
                    
            st.markdown('<p class="strategy-desc">12M 수익률 상위 30% 이내 & 1M 수익률 상위 10% 이내 (1M 순)</p>', unsafe_allow_html=True)
            st.dataframe(df_spec_t1.style.apply(apply_k200_styling, axis=1), 
                         use_container_width=True, hide_index=True,
