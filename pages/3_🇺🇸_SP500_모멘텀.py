...
with tab1:
    ...
    if not df_monthly.empty:
        ...
        month_label.markdown(f"**기준 선정일:** {base_date.strftime('%Y-%m-%d')}")
        ...
        # 💡 [해결 3] 트리플 교집합 표 렌더링 (최상단 배치)
        st.markdown("### 🏆 트리플 모멘텀 (3&6&12개월 교집합)")
        st.markdown('<p class="strategy-desc">3-1M, 6-1M, 12-1M 모두 상위 150위 이내인 최정예 종목 (6-1M 순 정렬)</p>', unsafe_allow_html=True)
        st.dataframe(df_triple_t1.style.apply(apply_korea_styling, axis=1), use_container_width=True, hide_index=True, column_order=col_order_triple, column_config=us_main_cfg)

        st.markdown("<hr>", unsafe_allow_html=True)
        c_l, c_r = st.columns(2)
        with c_l:
            st.markdown("#### 🔥 12-1M & 6-1M 교집합")
...
# (탭 2 데일리 순위는 생략하거나 위와 동일하게 add_naver_links만 적용)
with tab2:
    ...
    if os.path.exists(f_daily_path):
        ...
        st.info("데일리 데이터가 준비되었습니다. (상세 내역은 전체 순위표 참고)")
        st.dataframe(df_daily.head(100), use_container_width=True, column_config=us_main_cfg)
...
with tab3:
    with st.form("bt_settings_form_us", border=False):
        ...
        with c_chk:
            apply_timing = st.checkbox("🛑 마켓타이밍 적용", value=True, key='t3_chk_us')
        
        st.markdown("<hr style='margin: 10px 0px;'>", unsafe_allow_html=True)
        st.markdown("##### ✂️ 매수 순위 설정")
        c_ex1, c_ex2, c_ex3, c_ex4 = st.columns([1, 1, 1, 1])
        with c_ex1: r_s1 = st.slider("🔥 12-1&6-1 순위", 1, 30, (1, 5))
        with c_ex2: r_s2 = st.slider("🐎 6-1&3-1 순위", 1, 30, (1, 5))
        with c_ex3: r_tr = st.slider("🏆 트리플 순위", 1, 30, (1, 5)) # 💡 [해결 4] 트리플 순위 추가
        with c_ex4:
    ...
    if st.session_state.get('run_bt_state_us_v6', False):
        ...
        with st.spinner("백테스트 구동 중..."):
            ...
            df_res, df_trades = run_backtest_us_fast(
                ...
                r_s1, r_s2, r_tr, 150, 150, 150, spx_hist
            )
            if not df_res.empty:
                ...
                for col in s_cols:
                    ...
                    stats.append({"전략명": col, "CAGR": f"{cagr:.1f}%", "총수익률": f"{final_val-100:,.1f}%", "MDD": f"{mdd:.1f}%"})
                
                st.download_button("📥 종합 엑셀 리포트 다운로드", generate_excel_report_cached(tuple({'기간':f'{start_year}-{end_year}'}.items()), pd.DataFrame(stats), df_res, df_cum, df_trades), file_name="US_종합리포트.xlsx", use_container_width=True)
