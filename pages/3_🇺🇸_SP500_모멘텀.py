...
# 💡 [해결 3] 지수 종가 시차 에러 완벽 해결 (+1일 삭제 및 정밀 날짜 매칭)
...
# 💡 [해결 2] 예전 시스템 완벽 복원 (각 기간별로 지정된 N위까지만 잘라서 교집합 산출)
...
# 💡 [해결 2] 전용 백테스트 엔진 내장 (사용자 지정 컷오프 완벽 적용)
@st.cache_data(show_spinner=False)
def run_backtest_us_custom(df, start_year, end_year, ma_months, apply_timing, rank_s1, rank_s2, top_n_12, top_n_6, top_n_3):
    import yfinance as yf
    try:
...
with tab1:
    ...
    if not df_monthly.empty:
        ...
        with c_l:
            ...
            st.markdown('<p class="strategy-desc">12-1M & 6-1M 각각 10위 이내 교집합 종목 (6-1M 순)</p>', unsafe_allow_html=True)
        ...
        with c_r:
            ...
            st.markdown('<p class="strategy-desc">6-1M & 3-1M 각각 10위 이내 교집합 종목 (6-1M 순)</p>', unsafe_allow_html=True)

        # 💡 [요청 반영] 두 전략 모두에 겹치는 초강력 종목 노란색 표시
...
with tab3:
    ...
    # 💡 [해결 2] 백테스트 기간별 종목 추출 컷오프(순위) 조절 기능 탑재!
