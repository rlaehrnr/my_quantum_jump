...
# 맘 삼성 전략 자산군 (탭 2)
# ==========================================
# 필터: TIP·SPY가 11개월 이동평균 이격도 > 0 (둘 다 위)일 때만 공격 국면
SS_FILTER_ASSETS  = ['TIP', 'SPY']
# 공격: 12개월 이동평균 이격도 > 0인 것 모두 동일가중
SS_OFFENSE_ASSETS = ['FAS', 'SOXL', 'TQQQ', 'TMF']
# 방어: 5개월 이동평균 이격도 1위 1개 (절대모멘텀 미달이면 현금)
SS_DEFENSE_ASSETS = ['IEF', 'GLD', 'TBT']
...
def compute_signals_samsung(prices):
    ...
    """
    d11 = compute_ma_disparity(prices, 11)   # 필터
    d12 = compute_ma_disparity(prices, 12)   # 공격
    d5  = compute_ma_disparity(prices, 5)    # 방어
    ...
    # 준비도: 필터·공격·방어 자산의 해당 이격도가 모두 계산 가능해야 그 달 신호 유효.
    # 필수 자산 컬럼 자체가 누락이면 전 기간 미준비.
    ready = pd.Series(True, index=prices.index)
    if filt:
        ready &= d11[filt].notna().all(axis=1)
    if off:
    ...
    for m in prices.index:
        rec = {
            'disp11_TIP':  d11.loc[m, 'TIP']  if 'TIP'  in d11.columns else np.nan,
            'disp11_SPY':  d11.loc[m, 'SPY']  if 'SPY'  in d11.columns else np.nan,
            ...
            'disp5_IEF':   d5.loc[m, 'IEF']   if 'IEF'  in d5.columns else np.nan,
            'disp5_GLD':   d5.loc[m, 'GLD']   if 'GLD'  in d5.columns else np.nan,
            'disp5_TBT':   d5.loc[m, 'TBT']   if 'TBT'  in d5.columns else np.nan,
        }
        ...
        # 필터: TIP·SPY 둘 다 11M MA 이격도 > 0
        filter_pass = bool((d11.loc[m, filt] > 0).all())
        # 공격 통과 종목: 12M MA 이격도 > 0
        ...
        if filter_pass and len(off_pass) > 0:
            ...
            # 방어: 5M MA 이격도 1위 (절대모멘텀 > 0 통과해야 보유, 아니면 현금)
            disp_def = d5.loc[m, dfn]
            top = disp_def.idxmax()
