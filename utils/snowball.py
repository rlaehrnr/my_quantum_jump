...
#   · 방어 4종:  2개월 MA 이격도 상위 2종 동일가중(50:50)
...
# 411060(KRX금현물)은 공격에도 들어 있다 — 위험회피 국면에서도 고를 수 있게 방어에도 둔다.
# 2026-08 검토: 겹치는 55개월(2021-12~)에서 225130(금레버 2x·합성H)과 411060(금현물·환노출)의
# CAGR이 23.0%로 같은데 MDD는 -45.9% vs -26.4%, 월변동성은 1.65배 차이였다. 레버리지가
# 위험만 더 졌다. 그렇다고 225130을 빼는 '교체'로 가지 않은 건, 411060 상장(2021-12) 이전
# 구간에서는 방어 후보가 사실상 217770 하나만 남아 비교 자체가 불가능하기 때문이다.
# 후보만 늘리고 선택은 KO_DEF_WIN 이격도 순위에 맡긴다(되돌리기도 쉽다).
# ⚠️ 표본이 얇다 — 2022년 이후 방어 모드는 18개월뿐이고 개선의 t값은 1.08로 유의하지 않다.
KO_DEFENSE = ['217770', '225130', '455030', '411060']
...
# ==========================================================================
#   · 공격: 10종 중 12M 수익률 상위 5위 → 그중 12M 음수 종목은 제외하고
#           남은 '양수 모멘텀 승자'에 균등 재분배(듀얼모멘텀). 평소 5종 각 20%,
#           약세 브레스 땐 자동 집중.
#     ※ 2026-08 상위 4→5로 변경. 4종일 때 한 종목당 25%는 집중도가 높아
#       2026-07 같은 동반 급락에서 낙폭이 컸다.
#
#       ⚠️ 비교는 반드시 같은 기간으로 할 것. TOP_OFF를 올리면 준비도 게이트가
#          후보를 더 요구해 백테스트 시작월이 뒤로 밀린다(4종 2016-11,
#          5종 2017-04). 각자의 전체 기간으로 비교하면 기간 차이가 성과 차이로
#          둔갑한다.
#
#       공통기간(2017-04~, 112개월) 기준 실제 트레이드오프:
#              CAGR      MDD   Sharpe   최악월
#         4종  29.22%  -15.59%   1.58  -15.45%
#         5종  28.94%  -12.34%   1.66  -12.34%
#       → CAGR 0.28%p를 내고 MDD 3.25%p·최악월 3.11%p를 줄인다. 공짜가 아니다.
#       '종목을 늘리면 낙폭이 준다'는 방향은 4→5→6→7 내내 유지되지만,
#       6·7종은 표본이 59·55개월뿐이라 비교 자체가 약하다.
...
MAMTAX_TOP_OFF = 5   # 공격 상위 N종 (평소 각 20%). 2026-08에 4→5로 완화.
MAMTAX_TOP_DEF = 2
...
def compute_signals_mamtax(prices, us_prices):
    ...
    각 월 m에서 (모두 m월 말 데이터 기준 — 미래참조 없음):
      ...
      · 미발동 → 공격: 10종 중 12M 수익률 상위 MAMTAX_TOP_OFF위 → 12M 음수 제외 →
                 남은 승자에 균등 재분배(듀얼모멘텀).
    readiness: cond1 4종 6M 확정 AND 공격 12M 유효 종목이 MAMTAX_TOP_OFF개
               이상인 달부터 신호.
