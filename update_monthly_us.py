cat > update_monthly_us.py << 'PYEOF'
import pandas as pd
...
def get_top_us_stocks(market, limit=150):
    try:
        ...
        # FDR 최신 버전의 NASDAQ/NYSE 목록에는 시가총액 컬럼이 없고, 이미 시가총액 내림차순 정렬됨.
...
def generate_sp500(base_date, dates, start_date, base_date_str, invest_year, invest_month_str, invest_month_dash):
    ...
    if results:
        # 과거 파일과 동일하게 only_sp500_YYYY_MM.csv 로 통일 + 현재 월 기준 저장
...
def get_us_last_trading_day(today):
    """지난달 '실제 마지막 거래일'을 S&P500 지수(US500)로 찾는다. 실패 시 달력상 말일로 폴백."""
...
def main():
    ...
    base_date = get_us_last_trading_day(today)    # 지난달 마지막 실제 거래일
